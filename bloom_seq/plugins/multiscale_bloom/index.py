...
from bloom_seq.alphabets import Alphabet, DNA_ALPHABET
...
class MultiScaleBloomFilter:
    ...
    def __init__(
        ...
        Args:
            ...
            alphabet: Valid k-mer symbols (default DNA ACGT)
        """
    ...
    def n_scales(self) -> int:
        return len(self.k_sizes)

    @property
    def feature_dim(self) -> int:
        """Length of ``get_feature_vector`` (per-k stats + three aggregate ratios)."""
        return 5 * len(self.k_sizes) + 3
    ...
    def add_pathogenic_kmers(self, sequence: str, k: int = None):
        ...
        """
        sequence = self.alphabet.strip_ambiguity(sequence)
        ...
        for k_size in k_list:
            ...
            for i in range(len(sequence) - k_size + 1):
                ...
                if self.alphabet.is_kmer_valid(kmer):
    ...
    def add_pathogenic_kmer(self, kmer: str):
        ...
        if k in self.filters and self.alphabet.is_kmer_valid(kmer):
    ...
    def get_hit_positions(self, sequence: str, k: int = 8) -> List[Tuple[int, str]]:
        ...
        for i in range(len(sequence) - k + 1):
            kmer = sequence[i:i + k]
            if self.alphabet.is_kmer_valid(kmer) and kmer in self.filters[k]:
                hits.append((i, kmer))
                
        return hits
    
    def load_hbb_pathogenic_variants(self):
        """Seed filters with curated HBB pathogenic k-mers (reference plugin preset)."""
        from bloom_seq.plugins.clinvar_hbb.seeds import apply_hbb_pathogenic_preset

        apply_hbb_pathogenic_preset(self)
