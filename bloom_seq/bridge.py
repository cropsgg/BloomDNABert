...
class MutationAwarePooling(nn.Module):
    ...
    def __init__(self, d_model: int, d_bloom: int = 64):
        super().__init__()
...
class GatedCrossModalFusion(nn.Module):
    ...
    def __init__(self, d_model: int, bloom_summary_dim: int):
        super().__init__()
...
class BloomGuidedClassifier(nn.Module):
    ...
    End-to-end architecture:
        DNA Sequence
            |
            v
        [Bloom Filter] ---> Per-position signal [seq_len, n_scales]
            |                      |
            v                      v
        [Summary bloom_summary_dim]    [PositionalBloomEncoder]
            |                      |
            |               [seq_len, d_bloom]
            |                      |
            |     [Backbone] ---> Per-token hidden [seq_len, d_model]
            |           |                    |
            |           v                    v
