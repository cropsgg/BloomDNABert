...
from bloom_seq.bridge import BloomGuidedClassifier
from bloom_seq.alphabets import DNA_ALPHABET, Alphabet, validation_pattern
...
class HybridClassifier(nn.Module):
    ...
    Architecture:
    - Concatenate Bloom summary features with backbone pooled embedding
...
class HybridClassifierPipeline:
    ...
    def __init__(
        ...
        Args:
            ...
            alphabet: Allowed sequence symbols (default DNA)
        """
        self.bloom_filter = bloom_filter
        self.dnabert_wrapper = dnabert_wrapper
        self._alphabet = alphabet or DNA_ALPHABET
        self._seq_pattern = validation_pattern(self._alphabet, allow_ambiguity=True)
    ...
    def _validate_sequence(self, sequence: str) -> str:
        """Validate and sanitize a sequence against ``self._alphabet``."""
        if not sequence or not isinstance(sequence, str):
            raise ValueError("Sequence must be a non-empty string")
        sequence = self._alphabet.normalize(sequence)
        if len(sequence) < 10:
            raise ValueError(f"Sequence too short ({len(sequence)}). Minimum is 10.")
        if len(sequence) > self.MAX_SEQUENCE_LENGTH:
            raise ValueError(
                f"Sequence too long ({len(sequence)}). "
                f"Maximum is {self.MAX_SEQUENCE_LENGTH}."
            )
        if not self._seq_pattern.match(sequence):
            raise ValueError(
                f"Invalid characters; allowed alphabet {self._alphabet.name!r}: "
                f"{self._alphabet.symbols!r} (+ ambiguity {self._alphabet.ambiguity!r})."
            )
        return sequence
    ...
    def predict(self, sequence: str) -> Dict[str, float]:
        ...
        Args:
            sequence: Input sequence (length and alphabet per pipeline config)
...
class BloomGuidedPipeline:
    ...
    def extract_positional_features(
        ...
        Returns:
            hidden_states:  [num_tokens, d_model] per-token backbone hidden states
            bloom_signal:   [num_tokens, n_scales] token-aligned Bloom activation
            bloom_summary:  [feature_dim] flat Bloom features
        """
    ...
    def prepare_dataset(
        ...
        Returns:
            hidden_states:  [N, max_tokens, d_model]
            bloom_signals:  [N, max_tokens, n_scales]
            bloom_summaries: [N, bloom_summary_dim]
            labels:         [N]
        """
        all_hidden = []
        all_bloom_signal = []
    ...
    def predict(self, sequence: str) -> Dict[str, float]:
        ...
        Args:
            sequence: DNA sequence (10-5000 bp, alphabet-valid)
        """
        if not self.trained:
            raise ValueError("Model must be trained before prediction")
