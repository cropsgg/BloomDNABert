...
_FALLBACK = ["#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#aec7e8"]


def _symbol_color(ch: str) -> str:
    u = ch.upper()
    if u in _BASE_COLORS:
        return _BASE_COLORS[u]
    return _FALLBACK[ord(u) % len(_FALLBACK)]
...
class AttentionVisualizer:
    ...
    def create_nucleotide_importance_plot(
        ...
        colors = [_symbol_color(b) for b in sequence.upper()]
        ...
        fig = make_subplots(
            ...
            subplot_titles=(
                "Symbol importance (from attention)",
    ...
    def create_layer_comparison(
        ...
        """
        if layers is None:
            n = int(getattr(self.dnabert_wrapper, "num_layers", 12))
            if n <= 1:
                layers = [0]
            else:
                layers = [0, max(0, n // 2), n - 1]
